...
export const publicStore=row=>({name:row.store_name,nameCn:row.store_name_cn,address:row.address,taxNumber:row.tax_number,tableCount:row.table_count,taxMode:row.tax_mode,taxRate:row.tax_rate,logo:!!row.logo_png});
...
function parse(v,version){
 ...
 if(row.logoPng!==undefined&&!(row.logoPng===''||typeof row.logoPng==='string'&&/^data:image\/png;base64,[A-Za-z0-9+/]+={0,2}$/.test(row.logoPng)&&row.logoPng.length<=120000))return null;
 return row;
}
export async function settingsStaff(req,env,actor,readBody){const path=new URL(req.url).pathname;if(path!=='/api/staff/store')return null;
 ...
 const r=await env.DB.prepare(`UPDATE pos_store_config SET store_name=?,store_name_cn=?,address=?,tax_number=?,table_count=?,bank_label=?,bank_bin=?,bank_account=?,bank_name=?,transfer_prefix=?,tax_mode=?,tax_rate=?,github_url=?,logo_png=?,updated_at=?,updated_by=?,version=version+1 WHERE id=1 AND version=?`).bind(v.storeName,v.storeNameCn,v.address,v.taxNumber,v.tableCount,v.bankLabel,v.bankBin,v.bankAccount,v.bankName,v.transferPrefix,v.taxMode,v.taxRate,v.githubUrl,v.logoPng===undefined?old.logo_png:v.logoPng,new Date().toISOString(),actor.id,b.version).run();
 return r.meta.changes?json({ok:true,store:{...publicStore({...old,store_name:v.storeName,store_name_cn:v.storeNameCn,address:v.address,tax_number:v.taxNumber,table_count:v.tableCount,tax_mode:v.taxMode,tax_rate:v.taxRate,logo_png:v.logoPng===undefined?old.logo_png:v.logoPng}),bankLabel:v.bankLabel,bankBin:v.bankBin,bankAccount:v.bankAccount,bankName:v.bankName,transferPrefix:v.transferPrefix,githubUrl:v.githubUrl,version:b.version+1}}):json({ok:false,code:'STORE_CHANGED',message:'Cấu hình đã đổi ở máy khác'},409);
}
export async function logoResponse(env){const r=await getStore(env);if(!r?.logo_png)return new Response('Not found',{status:404});const data=r.logo_png.split(',')[1];if(!data)return new Response('Not found',{status:404});return new Response(Uint8Array.from(atob(data),x=>x.charCodeAt(0)),{headers:{'Content-Type':'image/png','Cache-Control':'no-store','X-Content-Type-Options':'nosniff'}})}
