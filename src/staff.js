...
export function paymentFor(row){
 ...
 const content=('PT '+row.code).replace(/[^A-Za-z0-9 ]/g,' ').replace(/\s+/g,' ').slice(0,25);
...
export async function handleStaff(req,env,deps){
 ...
 try{
  if(method==='POST'&&!deps.originValid(req))return error(403,'ORIGIN_REJECTED','Nguồn yêu cầu không hợp lệ');
  ...
  if(path==='/api/staff/voucher'&&method==='POST'){const b=await deps.body(req),v=deps.calculate(b),member=await memberById(env,b.memberId);if(b.memberId&&!member)return error(400,'INVALID_MEMBER','Không thấy hội viên');const offer=await resolveStaffVoucher(env,deps,b.voucherCode,v.subtotal,member);return result({subtotal:v.subtotal,discount:offer?.discount||0,total:v.subtotal-(offer?.discount||0),voucher:offer?.code||null})}
  if(path==='/api/staff/orders'&&method==='POST'){
   ...
   const value=deps.calculate(b),member=await memberById(env,b.memberId),vc=safe(b.voucherCode,32).toUpperCase();if(b.memberId&&!member)return error(400,'INVALID_MEMBER','Không thấy hội viên');
   const fingerprint=await deps.sha(JSON.stringify({...value,memberId:member?.id||null,voucherCode:vc}));
   ...
   const offer=await resolveStaffVoucher(env,deps,vc,value.subtotal,member),id=crypto.randomUUID(),time=now(),code='PT-'+time.slice(2,10).replaceAll('-','')+'-'+id.slice(0,10).toUpperCase();
   await env.DB.prepare(`INSERT INTO qr_orders(id,idem_key,fingerprint,token_hash,code,table_id,items_json,subtotal,discount,total,note,member_id,member_name,voucher_id,voucher_code,status,payment_status,source,version,kitchen_revision,created_at,updated_at,bank_bin,bank_account,bank_name,inventory_tracked) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'ACCEPTED','UNPAID','POS',1,1,?,?,?,?,?,1) ON CONFLICT(idem_key) DO NOTHING`).bind(id,idem,fingerprint,'STAFF_ONLY',code,value.table,JSON.stringify(value.items),value.subtotal,offer?.discount||0,value.subtotal-(offer?.discount||0),value.note,member?.id||null,member?.display_name||null,offer?.id||null,offer?.code||null,time,time,...deps.bankSnapshot(env)).run();
  ...
  const orderPath=path.match(/^\/api\/staff\/orders\/([a-f0-9-]{36})(?:\/(accept|append|cancel-unit|cancel|pay|split))?$/i);
  if(orderPath){const id=orderPath[1],action=orderPath[2];if(!uuid(id))return error(400,'INVALID_ID','Mã đơn không hợp lệ');if(method==='GET'&&!action)return overview(env,deps.hydrate,id);if(method!=='POST')return error(405,'METHOD_NOT_ALLOWED','Yêu cầu POST');const b=await deps.body(req),row=await getRow(env,id);
   ...
   if(action==='append'){
    const add=deps.calculate({table:row.table_id,items:b.items,note:''}),items=JSON.parse(row.items_json).concat(add.items);if(items.length>80||items.reduce((n,x)=>n+x.qty,0)>120)return error(400,'TOO_MANY_ITEMS','Đơn vượt giới hạn món');
    const subtotal=row.subtotal+add.subtotal,v=row.voucher_id?await env.DB.prepare('SELECT * FROM vouchers WHERE id=?').bind(row.voucher_id).first():null,discount=discounted(subtotal,v);
    const time=now();const r=await env.DB.prepare("UPDATE qr_orders SET items_json=?,subtotal=?,discount=?,total=?,last_delta_json=?,last_change_kind='ADD',payment_status='UNPAID',inventory_tracked=1,kitchen_revision=kitchen_revision+1,version=version+1,updated_at=? WHERE id=? AND status='ACCEPTED' AND version=? AND payment_status IN ('UNPAID','CUSTOMER_REPORTED')").bind(JSON.stringify(items),subtotal,discount,subtotal-discount,JSON.stringify(add.items),time,id,b.version).run();return r.meta.changes?overview(env,deps.hydrate,id):error(409,'ORDER_CHANGED','Tải lại đơn');
   }
   if(action==='cancel-unit'){
    ...
    const subtotal=items.reduce((s,x)=>s+x.qty*x.price,0);const v=row.voucher_id?await env.DB.prepare('SELECT * FROM vouchers WHERE id=?').bind(row.voucher_id).first():null;if(v&&subtotal>0&&subtotal<v.min_spend)return error(409,'VOUCHER_MIN','Hủy món làm đơn không đủ điều kiện voucher; hãy xử lý ưu đãi trước');const discount=discounted(subtotal,v),time=now();
    const r=await env.DB.prepare("UPDATE qr_orders SET items_json=?,subtotal=?,discount=?,total=?,last_delta_json=?,last_change_kind='CANCEL',payment_status='UNPAID',kitchen_revision=kitchen_revision+1,version=version+1,updated_at=?,status=CASE WHEN ?=0 THEN 'CANCELLED' ELSE status END WHERE id=? AND status='ACCEPTED' AND version=? AND payment_status IN ('UNPAID','CUSTOMER_REPORTED')").bind(JSON.stringify(items),subtotal,discount,subtotal-discount,JSON.stringify([delta]),time,items.length,id,b.version).run();return r.meta.changes?overview(env,deps.hydrate,id):error(409,'ORDER_CHANGED','Tải lại đơn');
   }
   if(action==='cancel'){
    if(!safe(b.reason,120))return error(400,'REASON_REQUIRED','Cần ghi lý do hủy');
    const time=now();const r=await env.DB.prepare("UPDATE qr_orders SET status='CANCELLED',version=version+1,updated_at=?,last_change_kind='CANCEL',last_delta_json=items_json,kitchen_revision=kitchen_revision+1,items_json='[]',subtotal=0,discount=0,total=0 WHERE id=? AND status='ACCEPTED' AND version=? AND payment_status IN ('UNPAID','CUSTOMER_REPORTED')").bind(time,id,b.version).run();return r.meta.changes?overview(env,deps.hydrate,id):error(409,'ORDER_CHANGED','Tải lại đơn');
   }
   if(action==='pay'){
   ...
   if(action==='split'){
    ...
    const counts=Array(old.length).fill(0),nowTime=now();let remainingDiscount=row.discount,remainingSubtotal=row.subtotal;
    const entries=parts.map((part,seq)=>{
     ...
     return {id:id+':'+(seq+1),items,subtotal,discount,total:subtotal-discount,seq:seq+1};
    });
    if(counts.some((n,i)=>n!==old[i].qty)||entries.some(x=>x.total<0)||entries.reduce((s,x)=>s+x.total,0)!==row.total)return error(400,'INVALID_SPLIT','Mỗi phần ăn phải thuộc đúng một bill');
    const statements=entries.map(x=>env.DB.prepare('INSERT INTO pos_bills(id,order_id,sequence,items_json,subtotal,discount,total,created_at,expected_version) VALUES(?,?,?,?,?,?,?,?,?)').bind(x.id,id,x.seq,JSON.stringify(x.items),x.subtotal,x.discount,x.total,nowTime,b.version));
  ...
  if(billPath&&method==='POST'){
   const b=await deps.body(req),id=billPath[1],v=await env.DB.prepare('SELECT b.*,o.bank_bin,o.bank_account,o.bank_name,o.code,o.status FROM pos_bills b JOIN qr_orders o ON o.id=b.order_id WHERE b.id=?').bind(id).first();
