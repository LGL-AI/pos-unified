...
test('customer order screen renders bank QR, saves PNG and requests staff without marking PAID',async()=>{
 ...
 const document={documentElement:{lang:'vi'},body:{style:{},appendChild(){}},querySelector:s=>s==='#app'?app:null,addEventListener:(name,fn)=>{listeners[name]=fn},createElement:type=>type==='canvas'?{width:0,height:0,getContext(){return{fillRect(){},fillText(){}}},toDataURL:()=> 'data:image/png;base64,ZmFrZQ=='}:{download:'',href:'',setAttribute(){},appendChild(){},click(){downloads.push(this.download)},remove(){}}};
 ...
 click({action:'payment-download'});assert.equal(downloads.length,1);assert.match(downloads[0],/EchoCoffee_QR_/);
 click({action:'request-staff'});await new Promise(r=>setTimeout(r,15));
 assert.equal(db.prepare('SELECT payment_status FROM qr_orders WHERE id=?').get(order.order.id).payment_status,'UNPAID');
 assert.equal(db.prepare('SELECT COUNT(*) AS n FROM pos_service_requests WHERE order_id=?').get(order.order.id).n,1);
 assert.doesNotMatch(app.innerHTML,/data-action="request-staff"/);
 db.close();
});
