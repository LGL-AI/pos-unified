...
test('customer order reaches POS; accepting and adding wait for payment before kitchen ticket',async()=>{
 ...
 assert.equal(a.order.status,'ACCEPTED');assert.equal(a.jobs.length,0);
 assert.equal((await call('/api/staff/orders/'+id+'/accept','POST',{version:1},token)).jobs.length,0);
 const add=await call('/api/staff/orders/'+id+'/append','POST',{version:a.order.version,items:[product(1)]},token);
 assert.equal(add.status,200,JSON.stringify(add));assert.equal(add.order.total,390000);assert.equal(add.jobs.length,0);
 ...
 const paid=await call('/api/staff/orders/'+id+'/pay','POST',{version:add.order.version,method:'BANK'},token);assert.equal(paid.status,200);assert.equal(paid.jobs.length,1);assert.equal(paid.jobs[0].kind,'NEW');assert.equal(paid.jobs[0].items.reduce((n,x)=>n+x.qty,0),3);
 assert.equal((await call('/api/staff/jobs/'+paid.jobs[0].id+'/claim','POST',{},token)).status,'CLAIMED');
 assert.equal((await call('/api/staff/jobs/'+paid.jobs[0].id+'/claim','POST',{},token)).status,409);
 assert.equal((await call('/api/staff/jobs/'+paid.jobs[0].id+'/status','POST',{status:'SENT'},token)).status,'SENT');
 assert.equal(db.prepare('SELECT COUNT(*) AS n FROM pos_kitchen_jobs').get().n,1);
...
test('customer service alert does not self-confirm bank payment or create kitchen jobs',async()=>{
 const {db,call,login,product}=system(),t=await login();
 ...
 const reported=await call('/api/orders/'+id+'/service','POST',{},undefined,{'x-order-token':q.orderToken});
 assert.equal(reported.requestPending,true);assert.equal(reported.order.paymentStatus,'UNPAID');assert.equal(db.prepare('SELECT COUNT(*) AS n FROM pos_kitchen_jobs WHERE order_id=?').get(id).n,0);
 const appended=await call('/api/staff/orders/'+id+'/append','POST',{version:accepted.order.version,items:[product()]},t);assert.equal(appended.status,200);assert.equal(appended.jobs.length,0);
 const paid=await call('/api/staff/orders/'+id+'/pay','POST',{version:appended.order.version,method:'BANK'},t);
