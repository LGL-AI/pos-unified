...
test('staff screen buttons create, append and split a four-unit order via the real API',async()=>{
 ...
 const app={innerHTML:''},connection={textContent:'',classList:{toggle(){}}},listeners={},formInputs={'#order-note':{value:''},'#voucher':{value:''},'#split-count':{value:'4'}};
 ...
 await click({add:'101'});for(let n=0;n<3;n++)await click({cartQty:'0',delta:'1'});
 assert.match(app.innerHTML,/Giỏ món \(4\)/);
 await click({action:'submit'});
 let row=db.prepare('SELECT id,total,status FROM qr_orders').get();assert.equal(row.total,520000);assert.equal(row.status,'ACCEPTED');assert.match(app.innerHTML,/Phiếu bếp/);
