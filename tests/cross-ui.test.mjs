...
test('counter stock reaches an open QR and handheld; QR order, POS changes and refunds reach all four screens',async()=>{
 ...
 await until(()=>customer.app.innerHTML.includes('data-add="101"'));
 assert.match(customer.app.innerHTML,/data-add="101"/);
 assert.match(handheld.app.innerHTML,/data-add="101"/);
 ...
 assert.match(customer.products.innerHTML,/data-add="101"/, 'Physical stock counting does not gate sales');
 counter.qty('10000');
 ...
 assert.equal(db.prepare('SELECT stock FROM pos_product_inventory WHERE product_id=?').get('101').stock,10);
 ...
 assert.equal(db.prepare('SELECT COUNT(*) AS n FROM pos_kitchen_jobs WHERE order_id=?').get(orderId).n,0);
 ...
 assert.equal(db.prepare('SELECT stock FROM pos_product_inventory WHERE product_id=?').get('101').stock,11);
 await counter.timers.get(10000)();
 ...
 await displayTimers.get(3000)();assert.match(screen.innerHTML,/Xin cảm ơn quý khách/);assert.doesNotMatch(screen.innerHTML,/Đã hoàn/);
